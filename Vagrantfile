...
Vagrant.configure(2) do |config|
  ...
  # taken from https://gist.github.com/diegopacheco/1f3d19f9645288febf83
  config.vm.box = "ubuntu/trusty64"
  config.vm.box_check_update = true
  config.vm.hostname = "scala-dev"
  ...
  config.vm.provision "shell", inline: <<-SHELL    
    ...
    # We also need finish installing lein, it will self install that's why we do lein for last
    #
